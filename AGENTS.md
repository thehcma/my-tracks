...
- ✅ **Create branches**: `gt create -am "descriptive commit message"`
- ✅ **Submit PRs**: `gt submit` (single) or `gt submit --stack` (stacked PRs)
...
- ✅ **Sync with remote**: `gt sync` to update local branches
- ✅ **Prune stale branches**: Periodically run `git fetch --prune && git branch -vv | grep ': gone]' | awk '{print $1}' | xargs -r git branch -D`
- ❌ **NEVER** use raw git commands for PR workflow (use `gt` instead)
- ❌ **NEVER** commit directly to main
- ❌ **NEVER** push directly to main
- Rationale: Graphite enables clean PR stacking, better code review, consistent workflow
