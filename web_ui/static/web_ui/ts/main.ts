...
async function requestDeviceLocations(): Promise<void> {
    ...
    const showResult = (text: string, delayMs = 2000): void => {
        if (btn) {
    ...
    try {
        ...
        const data: PaginatedResponse = await devicesResp.json();
        ...
        if (mqttDevices.length === 0) {
            ...
            showResult('No MQTT devices online');
            return;
        }
        ...
        showResult(`✅ Polled ${succeeded} device${succeeded !== 1 ? 's' : ''}`);
    } catch (err) {
